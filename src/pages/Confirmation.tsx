...
const Confirmation = () => {
  ...
  const { user } = useAuth();
  const { addBookedAppointment, generateMeetingLink, generateInvoiceNumber } =
    useBooking();
  const {
    therapist,
    package: pkg,
  ...
  useEffect(() => {
    if (therapist && pkg && date && time && !isAppointmentSaved) {
      ...
      const newAppointment: Appointment = {
        ...
        patientId: (user as any)?.id || "patient-001",
        patientName: (user as any)?.name || "Patient",
        patientPhoto: (user as any)?.avatar,
        date: date,
        time: formattedTime,
        duration: 50,
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-green-700 mb-3">
                    Use this link to join your video session at the scheduled
                    time. The link will be active 15 minutes before your
                    appointment.
                  </p>
                  <div className="flex items-center gap-2 bg-white border border-green-200 rounded-lg p-3">
                    ...
                    <code className="flex-1 text-sm text-green-800 break-all">
                      {meetingLink}
                    </code>
                    <button
            ...
            <div className="bg-healthcare-lavender/20 rounded-lg border border-healthcare-lavender/30 p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <p className="text-sm text-healthcare-text">
                    <strong>Join using the meeting link</strong> above, which
                    will be active 15 minutes before your session
                  </p>
                </div>
                <div className="flex items-start gap-3">
                  <div className="flex-shrink-0 w-6 h-6 rounded-full bg-green-500 text-white flex items-center justify-center text-sm font-bold">
                  ...
                  <p className="text-sm text-healthcare-text">
                    You can reschedule up to 2 times (minimum 24 hours before
                    session)
                  </p>
                </div>
              </div>
