    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
      <div className="bg-white rounded-2xl shadow-2xl border border-healthcare-border w-full max-w-xl max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-healthcare-border">
          <h2 className="text-base font-bold text-healthcare-text truncate">
            {ticket.subject}
          </h2>
          <button
            onClick={onClose}
            className="text-healthcare-text-muted hover:text-healthcare-text transition-colors bg-transparent border-none cursor-pointer p-1"
          >
            <svg
              className="w-5 h-5"
              fill="none"
        ...
        <div className="overflow-y-auto flex-1 p-6 space-y-4">
          <div className="text-sm">
            ...
            <p className="text-healthcare-text-muted text-xs">
              {ticket.userEmail}
            </p>
          ...
          {ticket.adminReply && (
            <div className="p-4 bg-brand-blue/5 rounded-xl border border-brand-blue/15 text-sm text-healthcare-text">
              <p className="text-xs font-bold text-brand-blue mb-1.5">
                Admin Reply
              </p>
              {ticket.adminReply}
            </div>
        ...
        <div className="px-6 py-4 border-t border-healthcare-border flex justify-end gap-2">
          <button
            onClick={onClose}
            className="px-4 py-2 text-sm font-bold text-healthcare-text bg-healthcare-surface border border-healthcare-neutral/20 rounded-xl hover:bg-white transition-all cursor-pointer"
          >
            Cancel
          </button>
          <button
                ...
                <tbody className="divide-y divide-healthcare-border">
                  ...
                  {(data.items || []).map((t) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <span
                          className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${STATUS_BADGE[t.status] ?? "bg-gray-100 text-gray-600"}`}
                        >
