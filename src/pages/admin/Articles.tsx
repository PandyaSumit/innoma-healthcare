...
export default function Articles() {
  ...
  const params =
    filter === 'all' ? { page, limit: 20 } : { page, limit: 20, published: filter === 'published' };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-healthcare-text-muted mt-1">Manage blog posts and published content.</p>
        </div>
        <Link
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-healthcare-border bg-healthcare-surface/40">
                    <th className="text-left px-5 py-3.5 text-xs font-bold text-healthcare-text-muted uppercase tracking-wider">Title</th>
                    <th className="text-left px-5 py-3.5 text-xs font-bold text-healthcare-text-muted uppercase tracking-wider hidden md:table-cell">Author</th>
                ...
                <tbody className="divide-y divide-healthcare-border">
                  ...
                  {data.items.map((a) => (
                    <tr key={a.id} className="hover:bg-healthcare-surface/20 transition-colors">
                      <td className="px-5 py-4">
                        <p className="font-bold text-healthcare-text line-clamp-1">{a.title}</p>
                        {a.excerpt && (
                          <p className="text-xs text-healthcare-text-muted line-clamp-1 mt-0.5">{a.excerpt}</p>
                        )}
                      </td>
                      <td className="px-5 py-4 text-healthcare-text hidden md:table-cell">{a.authorName}</td>
                      <td className="px-5 py-4 text-healthcare-text-muted hidden sm:table-cell">
                        {a.publishedAt ? new Date(a.publishedAt).toLocaleDateString() : '—'}
                      </td>
                      <td className="px-5 py-4">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${a.isPublished ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600'}`}>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2 justify-end">
                          <button
                            onClick={() => togglePublish.mutate({ id: a.id, published: a.isPublished })}
