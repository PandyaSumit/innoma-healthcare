...
function SummaryCard({
  ...
  return (
    <div className="bg-white rounded-2xl border border-healthcare-border p-5 shadow-clinical">
      <p className="text-xs font-bold text-healthcare-text-muted uppercase tracking-wider mb-2">
        {label}
      </p>
      <p className="text-2xl font-bold text-healthcare-text">{value}</p>
      {sub && (
        <p
          className={`text-xs font-semibold mt-1 ${positive ? "text-emerald-600" : "text-red-500"}`}
        >
          {sub}
        </p>
      )}
    ...
    <div className="space-y-1.5">
      {data.slice(-14).map((d) => (
        <div key={d.date} className="flex items-center gap-3">
          <span className="text-xs text-healthcare-text-muted w-20 flex-shrink-0">
            {d.date.slice(5)}
          </span>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-healthcare-text-muted mt-1">
          Revenue overview and session analytics.
        </p>
      </div>
      ...
      {summary.isLoading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          ...
          <SummaryCard
            ...
            positive={(summary.data.growthPercent ?? 0) >= 0}
          />
        </div>
      ) : null}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-healthcare-border bg-healthcare-surface/40">
                  ...
                  <th className="text-right px-5 py-3.5 text-xs font-bold text-healthcare-text-muted uppercase tracking-wider">
                    Sessions
                  </th>
                  <th className="text-right px-5 py-3.5 text-xs font-bold text-healthcare-text-muted uppercase tracking-wider">
                    Revenue
                  </th>
                  <th className="text-right px-5 py-3.5 text-xs font-bold text-healthcare-text-muted uppercase tracking-wider hidden md:table-cell">
                    Avg Rating
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-healthcare-border">
                {((therapists.data as any) || []).map((t: any) => (
                  <tr
                    key={t.therapistId}
                    className="hover:bg-healthcare-surface/20 transition-colors"
                  >
                    <td className="px-5 py-4 font-bold text-healthcare-text">
                      {t.therapistName}
                    </td>
                    <td className="px-5 py-4 text-right text-healthcare-text">
                      {t.sessions}
                    </td>
                    <td className="px-5 py-4 text-right font-bold text-healthcare-text">
