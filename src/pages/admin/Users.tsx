    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
      <div className="bg-white rounded-2xl shadow-2xl border border-healthcare-border w-full max-w-lg">
        <div className="flex items-center justify-between px-6 py-4 border-b border-healthcare-border">
          <div>
            <h2 className="text-base font-bold text-healthcare-text">Send Email</h2>
            <p className="text-xs text-healthcare-text-muted mt-0.5">To: {user.name} ({user.email})</p>
          </div>
        ...
        <form onSubmit={formik.handleSubmit} className="p-6 space-y-4">
          <div className="space-y-1.5">
            <label className="block text-sm font-semibold text-healthcare-text">Subject</label>
            <input
              name="subject"
            ...
            {formik.touched.subject && formik.errors.subject && (
              <p className="text-red-500 text-xs font-bold">{formik.errors.subject}</p>
            )}
          </div>

          <div className="space-y-1.5">
            <label className="block text-sm font-semibold text-healthcare-text">Message</label>
            <textarea
              name="message"
            ...
            {formik.touched.message && formik.errors.message && (
              <p className="text-red-500 text-xs font-bold">{formik.errors.message}</p>
            )}
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={send.isPending} className="flex-1 py-2.5 bg-brand-blue text-white rounded-xl text-sm font-bold hover:opacity-90 transition-all disabled:opacity-50 disabled:cursor-not-allowed border-none cursor-pointer">
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-healthcare-text-muted mt-1">Browse and manage patient accounts.</p>
      </div>

      {/* Filters */}
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="flex gap-2 flex-wrap">
          {(['all', 'registered', 'free_assessment', 'paid_session'] as const).map((s) => (
            <button
              key={s}
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-healthcare-border bg-healthcare-surface/40">
                    <th className="text-left px-5 py-3.5 text-xs font-bold text-healthcare-text-muted uppercase tracking-wider">User</th>
                    <th className="text-left px-5 py-3.5 text-xs font-bold text-healthcare-text-muted uppercase tracking-wider hidden md:table-cell">Joined</th>
                    <th className="text-left px-5 py-3.5 text-xs font-bold text-healthcare-text-muted uppercase tracking-wider hidden sm:table-cell">Sessions</th>
                    <th className="text-left px-5 py-3.5 text-xs font-bold text-healthcare-text-muted uppercase tracking-wider">Stage</th>
                ...
                <tbody className="divide-y divide-healthcare-border">
                  ...
                  {data.items.map((u) => (
                    <tr key={u.id} className="hover:bg-healthcare-surface/20 transition-colors">
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-bold text-healthcare-text">{u.name}</p>
                            <p className="text-xs text-healthcare-text-muted">{u.email}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${STAGE_BADGE[u.stage]}`}>
                          {STAGE_LABEL[u.stage]}
                        </span>
                      </td>
                      <td className="px-5 py-4">
          ...
          {data.totalPages > 1 && (
            <div className="flex items-center justify-between text-sm">
              <p className="text-healthcare-text-muted">
                {data.total.toLocaleString()} users · Page {data.page} of {data.totalPages}
              </p>
