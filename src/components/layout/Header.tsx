          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex-shrink-0">
              <a href="#home" className="flex items-center gap-2 group cursor-pointer no-underline">
                <span className={`text-2xl font-bold tracking-tight transition-colors ${isScrolled ? 'text-brand-blue' : 'text-white'}`}>
      ...
      >
        <div className="flex flex-col h-full">
          <div className="p-6 flex items-center justify-between border-b border-gray-100">
            <span className="text-sm font-bold text-brand-blue">Menu</span>
            <button 
              onClick={() => setIsMobileMenuOpen(false)}
              className="p-2 text-brand-blue hover:text-brand-orange transition-colors cursor-pointer"
            >
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <nav className="flex flex-col p-6 gap-4">
            {navLinks.map((link) => (
              <a
                ...
                href={link.href}
                onClick={() => setIsMobileMenuOpen(false)}
                className="text-lg font-semibold text-brand-blue hover:text-brand-orange transition-colors no-underline"
              >
                {link.name}
              </a>
            ))}
          ...
          <div className="mt-auto p-6 bg-gray-50 flex flex-col gap-3">
            <button className="w-full py-3 text-sm font-bold text-brand-blue border border-brand-blue/20 rounded-md hover:bg-white transition-colors">
              Sign In
            </button>
